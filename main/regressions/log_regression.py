...
def ellipsoid(x: float, y: float, params: Tuple[float, float, float, float, float]) -> float:
    ...
    :param params: расстояние от точки (x,y) до эллипсоида f(x, y), f(x, y) = 0 <- точка принадлежит эллипсоиду
    :return: значение уравнения эллипсоида в точке
    """
...
def log_reg_ellipsoid_test_data(params: Tuple[float, float, float, float, float],
    ...
    """
    Генератор тестовых данных для логистической регрессии для эллипсоида. Чтобы понять что тут просходит, просто  _debug_mode = True
    :param params:
    :param arg_range:
    :param rand_range:
    :param n_points:
...
def log_reg_test_data(k: float = -1.5, b: float = 0.1, arg_range: float = 1.0,
    ...
    """
    Генератор тестовых данных для логистической регрессии прямой. Чтобы понять что тут просходит, просто  _debug_mode = True
    :param k:
    :param b:
    :param arg_range:
    :param rand_range:
...
class LogisticRegression:
    ...
    def __str__(self):
        ...
        :return: JSON совместимая строка
        """
        return f"\t\t{{\n" \
    ...
    def gradient_descend(self, features, groups, thetas):
        """
        Функция реализует градиентный спуск по формуле X^T * ((sigmoid(X * thetas) - groups)
        для обучения логистической регрессии
        :param features: признаки групп, записанные в виде столбцов
        :param groups: вектор столбец принадлежности групп
        :param thetas: вектор-строка весов
        :return: значение градиента
        """
        z = np.dot(features, thetas)
        # return (np.dot(features.T, (sigmoid(z) - groups))) * one_over_groups
        return np.dot(features.T, sigmoid(z) - groups)
    ...
    def train(self, features: np.ndarray, groups: np.ndarray) -> None:
        ...
        """
        """
        x = (1 | features) => w.size = features.shape[1] + 1
        """
        ...
        for i in range(self._max_train_iters):
            ...
            self._thetas -= self._learning_rate * self.gradient_descend(x, groups, self._thetas)
...
def lin_keras_test():
    ...
    model.add(keras.layers.Dense(1, input_dim=features.shape[1], activation='sigmoid'))
    ...
    draw_logistic_data(features, group, np.hstack((w_s[1], w_s[0].reshape(1, features.shape[1])[0])))


def non_lin_keras_test():
    features, group = log_reg_ellipsoid_test_data((0.08, -0.08, 1.6, 1.0, 1.0))
    ...
    w_s = model.get_weights()
    thetas = np.hstack((w_s[1], w_s[0].reshape(1, features.shape[1])[0]))
