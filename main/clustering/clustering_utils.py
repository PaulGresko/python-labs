...
def draw_cluster(cluster: np.ndarray, rgb: Union[Tuple[int, int, int], str] = (255, 0, 0), axes=None, show=False):
    ...
    if cluster.shape[1] == 3:
        if isinstance(rgb, str):
            axes.plot(cluster[:, 0], cluster[:, 1], cluster[:, 2], '.', color=rgb)
        if isinstance(rgb, tuple):
            axes.plot(cluster[:, 0], cluster[:, 1], cluster[:, 2], '.', color=pack_color_code(*rgb))
    if show:
        plt.show()
...
def draw_clusters(clusters: List[np.ndarray],
    ...
    fig, axes = plt.figure().add_subplot(projection='3d'), plt.gca()#plt.subplots(1, projection='3d')
    legend = []
    ...
    if cluster_centers is not None:
        ...
        for center, color in zip(cluster_centers, marker_colors):
            ...
            if center.size == 3:
                axes.plot(center[0], center[1], center[2], marker='*', markersize=10, color=color)
